...
const Booking = () => {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const guestMatch =
      b.guest_id?.name?.toLowerCase().includes(searchString) ||
      b.guest_id?.contact?.includes(searchString);
    ...
    <>
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          ...
          <p className="text-on-surface-variant text-sm font-medium mt-3">
            Manage all guest bookings and current stays
          </p>
        </div>
      ...
      <section className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8 border-b border-outline/10 pb-4">
        <div className="flex overflow-x-auto w-full xl:w-auto scrollbar-hide pb-2 xl:pb-0 gap-2">
          {["All", "Checked-In", "Booked", "Checked-Out", "Cancelled"].map(
            (item) => {
              let countLabel = "";
      ...
      <div className="bg-surface rounded-none overflow-x-auto shadow-sm border border-outline/20 mb-8">
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead className="bg-surface-container">
            <tr>
              <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-on-surface-variant">
                Guest
              </th>
              <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-on-surface-variant">
                Room
              ...
              <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-on-surface-variant text-right">
                Actions
              </th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              ...
              filteredBookings.map((booking) => {
                ...
                return (
                  <tr
                    key={booking._id}
                    onClick={() => navigate(`/app/receptionist/bookings/${booking._id}`)}
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20 overflow-hidden text-primary font-serif font-bold">
                          {booking.guest_id?.name
                            ? booking.guest_id.name.charAt(0).toUpperCase()
                            : "?"}
                        </div>
                        <div>
                          <p className="font-medium text-on-surface leading-none">
                            {booking.guest_id?.name || "Unknown"}
                          </p>
                    ...
                    <td className="px-6 py-5 font-medium text-on-surface">
                      {booking.room_id
                        ? `${booking.room_id.room_category} ${booking.room_id.room_number}`
                        : "Unassigned"}
                    </td>
                    ...
                    <td className="px-6 py-5 text-sm text-on-surface-variant">
                      {new Date(booking.expected_checkout).toLocaleDateString(
                        "en-US",
                        { month: "short", day: "numeric", year: "numeric" },
                      )}
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold ${sConf.bg} ${sConf.text} ${sConf.border} border`}
                      >
                        <span
                      ...
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        {booking.status === "booked" && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleStatusAction(booking._id, "booked");
                            }}
                        ...
                        {booking.status === "checked-in" && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleStatusAction(booking._id, "checked-in");
                            }}
